document.addEventListener("DOMContentLoaded", function() {
    ...
    function handleCardClick(event) {
        ...
        flipCard(card);
        // 두 카드가 뒤집혔을 때 처리
        if (flippedCards.length === 2) handlePairComparison();
    }
    // 클릭 무시해야 할 상황 체크
    function shouldIgnoreClick(card) {
        return isProcessing || card.classList.contains("flipped") || card.classList.contains("matched") || flippedCards.includes(card);
    }
    // 카드 뒤집기
